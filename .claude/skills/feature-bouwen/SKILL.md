...
1. **Schemabeslissing.** Leg de story en acceptatiecriteria vast in
   `docs/project/stories/<nummer>-<naam>.md`, met expliciet welke velden en types nodig zijn. Abstraheer
...
2. **Kies de service, dan isoleer.** Een feature hoort bij precies één service (ADR-0002).
   Welke services er zijn en waar ze staan, leest `docs/project/architectuur/stack-profiel.md` §Topologie voor — is dat
...
3. **Schrijf de ene bron, zoals dit project 'm heeft vastgelegd in
   `docs/project/architectuur/stack-profiel.md` (§De ene bron).** Bestaat dat bestand nog niet: dat is de
   ...
   `docs/project/architectuur/stack-profiel.md` en vul 'm in vóór je verdergaat.
   ...
   uitziet is nog niet uitgewerkt en is dus precies wat `docs/project/architectuur/stack-profiel.md` §De ene bron per
...
4. **Genereer de keten** van de service waar je in werkt: het API-schema uit de ene bron, en
   ...
   in `docs/project/architectuur/stack-profiel.md` §Contractgeneratie; zegt dat "nee" (geen generatie), dan vervalt deze
...
- **`openapi-typescript` trekt via `@redocly/openapi-core` soms een kwetsbare `js-yaml`-versie
  ...
  productie-runtime.
- **ORM-specifieke valkuilen horen niet hier maar in `docs/project/architectuur/stack-profiel.md`** — lazy-loading van
