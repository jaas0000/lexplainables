...
// De TOTP-lib die `pyotp` matcht: default 30s window, 6 digits, base32. `otplib` is één van
// de weinige TS-libs die dat contract deelt zonder configuratie.

test("2FA-activering: gebruiker koppelt en logt in met code", async ({
  page,
  context,
  request,
}) => {
  ...
  // vervolgens met dat nieuwe secret). API_TOKEN + X-User-Id via _helpers.
  const beginRes = await request.post(
    `${process.env.API_BASE_URL ?? "http://localhost:8000"}/v1/auth/2fa/begin`,
    {
      headers: {
        Authorization: `Bearer ${process.env.API_TOKEN ?? "local-dev-api-token-lexplainables"}`,
        "X-User-Id": "beheerder",
      },
    },
  );
  ...
  // Cleanup: 2FA uitschakelen zodat vervolgtests niet vastlopen.
  const uitschakelCode = authenticator.generate(secret);
  await request.post(
    `${process.env.API_BASE_URL ?? "http://localhost:8000"}/v1/auth/2fa/uitschakel`,
    {
      headers: {
        Authorization: `Bearer ${process.env.API_TOKEN ?? "local-dev-api-token-lexplainables"}`,
        "X-User-Id": "beheerder",
      ...
      data: JSON.stringify({ totp: uitschakelCode }),
    },
  );
});
