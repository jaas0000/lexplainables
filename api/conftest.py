...
def maak_test_engine(*metadatas: MetaData, tmp_path) -> AsyncEngine:
    ...
    if pg_sync and pg_async:
        sync_engine = create_engine(pg_sync)
        for metadata in metadatas:
            metadata.drop_all(sync_engine)
            metadata.create_all(sync_engine)
        sync_engine.dispose()
        return create_async_engine(pg_async)
    ...
    for metadata in metadatas:
        metadata.create_all(sync_engine)
    sync_engine.dispose()
    return create_async_engine(f"sqlite+aiosqlite:///{db_pad}")
...
def sync_engine_voor(async_engine: AsyncEngine) -> Engine:
    ...
    blijft ongewijzigd; de sync-engine moet door de aanroeper gedisposed worden.
    """
    url = async_engine.url
    if url.drivername == "sqlite+aiosqlite":
        return create_engine(f"sqlite:///{url.database}")
    if url.drivername == "postgresql+asyncpg":
        return create_engine(url.set(drivername="postgresql"))
