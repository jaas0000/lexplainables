...
Wat: applicatie-brede instellingen die beheerders live kunnen aan-/uitzetten (bijv. `capture_llm_calls`) zonder redeploy; TTL-cache voor lees-latentie; PATCH-endpoint voor selectieve updates.
Waarom: eigen domein omdat runtime-instellingen niet in code horen (env-vars zijn deploy-tijd) en niet in per-feature-config passen — een centrale key/value-tabel met defaults en type-checks is de goede vorm.
