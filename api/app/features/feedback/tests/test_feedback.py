...
def test_verwijderen(client):
    ...
    response = client.delete(f"/v1/admin/feedback/{feedback_id}")
    assert response.status_code == 204

    lijst = client.get("/v1/admin/feedback").json()
    assert lijst["totaal"] == 0


def test_verwijderen_onbekend_id_geeft_404(client):
    response = client.delete("/v1/admin/feedback/999")
...
def test_paginering(client):
    ...
    eerste_pagina = client.get("/v1/admin/feedback", params={"offset": 0, "limit": 2}).json()
    ...
    tweede_pagina = client.get("/v1/admin/feedback", params={"offset": 2, "limit": 2}).json()
...
def test_ongelezen_aantal_voor_en_na_markeer_gezien(client):
    ...
    aantal = client.get("/v1/admin/feedback/ongelezen-aantal").json()
    assert aantal["aantal"] == 2

    response = client.post("/v1/admin/feedback/markeer-gezien", json={})
    assert response.status_code == 204
    aantal = client.get("/v1/admin/feedback/ongelezen-aantal").json()
...
def test_markeer_gezien_met_expliciete_tot_beschermt_tegen_race_conditie(client):
    ...
    item_a = client.get("/v1/admin/feedback").json()["items"][0]
