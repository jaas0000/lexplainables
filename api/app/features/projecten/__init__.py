...
Wat: analisten maken analyses aan met één of meer bronnen (bwb_id + artikel + lid); een background-job orkestreert act2/act3 via `engine/orchestrator`; SSE geeft live status; rapport-endpoint levert eindresultaat + Markdown-download; LLM-calls-log koppelt aan `llm_calls`.
Waarom: eigen domein voor analyse-orkestratie — status-machine, background-task-lifecycle, SSE-stream, human-in-the-loop akkoord/afwijzen. Los van annotatie (dat is post-analyse werkplek) en van de engine zelf (dat is stateless orkestratie-code).
...
Tabellen:
  - analyses: id + naam + client_id + status (actief/wacht_op_review/klaar/fout) + fase + bronnen (JSON) + begrippenlijst (JSON, optioneel) + human_in_the_loop + resultaat (JSON) + timestamps.
