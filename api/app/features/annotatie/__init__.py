...
Wat: analisten annoteren wetsartikelen met JAS-klasse-elementen; per element een human-beslissing (goedkeuren/bewerken/afwijzen/opmerking) met levenscyclus, plus een append-only auditlog.
Waarom: annotatie is de kern-analyse-stap en heeft eigen levensduur, status-berekening en auditregels — geen slice van projecten (die orkestreert enkel), geen slice van wetcatalogus (die is puur lookup).
...
Tabellen:
  - annotatie_documenten: werkdocument per (werkgebied, bwb_id, artikel, lid) met JSON-lijst van elementen.
...
Beslissingen:
  ...
  - Feature-bouwen regel 8: `GELDIGE_JAS_KLASSEN` staat in `shared/validation.py` — tweede onafhankelijke gebruiker naast `engine/validation.py`.
