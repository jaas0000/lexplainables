...
- [x] Na elke succesvolle login wordt de gebruiker doorgestuurd naar `/disclaimer` als hij de
      disclaimer nog niet heeft geaccepteerd; na acceptatie gaat hij door naar de oorspronkelijke
      bestemming.
- [x] `/disclaimer` toont de drie waarschuwingsblokken met de volgende koppen en bodytekst:
...
- [x] Op `/disclaimer` staat een "Begrepen — doorgaan"-knop die de disclaimer accepteert en de
      gebruiker doorstuurt. Heeft de gebruiker al eerder geaccepteerd, dan staat er een
      "Terug"-link in plaats van de knop.
- [x] De acceptatie wordt bijgehouden via een httpOnly-cookie (`disclaimer_geaccepteerd=1`), zodat
      de check server-side kan plaatsvinden zonder een API-aanroep.
- [x] Elke pagina toont wanneer de gebruiker ingelogd is een horizontale strip bovenaan de header
...
- [x] De strip gebruikt de bestaande CSS-variabele `--waarschuwing` voor de achtergrondkleur (geen
      losse hex).
- [x] `/disclaimer` is ook bereikbaar als leespagina nadat de disclaimer al geaccepteerd is (de
