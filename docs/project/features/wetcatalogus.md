...
beheerders beheren een gecureerde lijst van wetten (naam + bwb_id) via CRUD; analisten lezen de lijst en de structuur (artikelen per wet); admin-`resolve` kan een bwb_id opzoeken.
...
**Grens:** het domein zelf slaat alleen naam + bwb_id op; de structuur (artikelen/leden) en actuele tekst komen live uit een externe bron — geen dubbele opslag.
...
- Story 010 §Databron: catalogus in eigen tabel, structuur (artikelen) live opgehaald; geen caching van artikelen om drift met bronnen te voorkomen.
