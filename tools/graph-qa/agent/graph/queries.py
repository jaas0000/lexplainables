"""Geparametriseerde SPARQL-bouwers voor de kennisgraaf (werkwijze-story 041).

De invoer wordt gevalideerd/ge-escaped zodat het model geen SPARQL kan injecteren via een
tool-argument.

Grotendeels een poort van `wetsanalyse-ai/tools/graph-qa/agent/graph/queries.py`, met twee
schemacorrecties — gevonden via live-verificatie tegen dit project se eigen `tools/bwb-import`-
data (de referentie se importer schrijft een ander schema):

- **Geen `bwb:jci`-predicaat.** Dit project se importer schrijft de wetten.overheid.nl-
  jci-vindplaats als `owl:sameAs`, niet als een los `bwb:jci`-literal. Elke `?jci`-binding hier
  leest daarom `owl:sameAs`.
- **`bwb:heeftOnderdeel+` in plaats van `bwb:bevat`** voor geneste onderdelen, en `eli:has_part`
  (omgekeerd) in plaats van `bwb:bevat` voor "wat bevat deze node" — dit project se importer kent
  geen generiek `bwb:bevat`-predicaat. Zelfde patroon als `api/app/features/annotatie/
  graphdb.py`, dat dit al eerder tegen dezelfde data verifieerde (`bwb:heeftOnderdeel+`
  property-path i.p.v. de platte `bwb:bevat`-kopie).
"""
...
# Eigen IRI-ruimte — filter hierop om owl:sameAs-tweelingen (wetten.overheid.nl) buiten
# tellingen/resultaten te houden.
NS = BASIS
...
def fts(query: str, limit: int = 10) -> str:
    ...
    return (
        PREFIXES
        + f"""SELECT ?node ?score ?label ?tekst WHERE {{
...
def list_regelingen() -> str:
    return (
        PREFIXES
        + f"""SELECT DISTINCT ?regeling ?citeertitel ?soort WHERE {{
...
def get_artikel(bwb_id: str, artikel: str) -> str:
    ...
    a/b/c direct onder het artikel — en ontbraken anders volledig. Onderdelen die onder een *lid*
    hangen komen bewust niet mee: bij een definitieartikel zijn dat er tientallen en dan kapt de
    8000-tekenslimiet het resultaat af. Daarvoor is `get_lid`, dat ze wél levert.
    """
    iri = artikel_iri(bwb_id, artikel)
    return (
        PREFIXES
        + f"""SELECT ?tekst ?jci ?lid ?lidnummer ?lidtekst ?onderdeel ?onderdeeltekst WHERE {{
  OPTIONAL {{ <{iri}> bwb:tekst ?tekst }}
  OPTIONAL {{ <{iri}> owl:sameAs ?jci }}
  OPTIONAL {{
...
def get_lid(bwb_id: str, artikel: str, lid: str) -> str:
    ...
    t. `bwb:heeftOnderdeel+` (property-path) haalt ook geneste onderdelen op.
    """
    iri = lid_iri(bwb_id, artikel, lid)
    # De onderdelen in één cel (GROUP_CONCAT) i.p.v. één rij per onderdeel: anders herhaalt de
    # lidtekst zich per onderdeel en loopt een definitielid tegen de 8000-tekenslimiet, waarna
    # juist de laatste onderdelen wegvallen. De subquery met ORDER BY houdt de volgorde a, b, c, …
    return (
        PREFIXES
        + f"""SELECT ?nummer ?tekst ?jci
  ...
  OPTIONAL {{
    {{ SELECT ?regel WHERE {{
        ...
        #
        # Zonder de datumstaart (&z=…&g=…): die is voor 25 onderdelen ~700 tekens aan herhaling
        # en staat al in de jci van het lid hierboven. Wat overblijft is een geldige jci-verwijzing.
        # STR(?oj) is verplicht: ?oj is een IRI (owl:sameAs), en CONCAT() eist stringliteralen —
        # zonder STR() faalt de BIND stil voor elke rij en blijft ?regel (en dus de hele
        # GROUP_CONCAT) leeg, zonder foutmelding. Live geverifieerd tegen dit project se data (de
        # referentie had hier geen STR() nodig omdat haar `bwb:jci` al een literal was, geen IRI).
        BIND(IF(BOUND(?oj),
                IF(CONTAINS(STR(?oj), "&z="), STRBEFORE(STR(?oj), "&z="), STR(?oj)),
                "") AS ?ojk)
...
def get_bepaling(bwb_id: str, nummer: str) -> str:
    ...
    "25", "22a") én divisies/decimale nummers ("9.1") van beleidsregels/circulaires (bv. de
    Leidraad Invordering 2008), waar het artikel/lid-IRI-patroon niet opgaat."""
    return (
        PREFIXES
        + f"""SELECT ?nummer ?tekst ?label ?jci WHERE {{
...
def get_regeling_info(bwb_id: str) -> str:
    ...
    return (
        PREFIXES
        + f"""SELECT ?citeertitel ?opschrift ?afkorting ?soort
...
def follow_verwijzingen(bwb_id: str, artikel: str, lid: str | None = None) -> str:
    ...
    return (
        PREFIXES
        + f"""SELECT ?ankerTekst ?naar ?soort ?doelSoort WHERE {{
...
def referenced_by(bwb_id: str, artikel: str) -> str:
    ...
    return (
        PREFIXES
        + f"""SELECT DISTINCT ?regeling ?citeertitel WHERE {{
...
def resolve_begrip(term: str) -> str:
    return (
        PREFIXES
        + f"""SELECT DISTINCT ?concept ?label ?related WHERE {{
...
def count_by_type() -> str:
    return (
        PREFIXES
        + f"""SELECT ?type (COUNT(DISTINCT ?s) AS ?aantal) WHERE {{
  ?s a ?type .
...
def context(bwb_id: str, artikel: str, lid: str | None = None) -> str:
    ...
    Levert per relatie-soort (?relatie) een rij: de bepaling zelf (label/tekst/jci), de
    bevattende structuurdelen, de leden, de uitgaande verwijzingen en wie naar het artikel
    verwijst. Zo ziet het model de bepaling ingebed in samenhang i.p.v. losse triples. Eén
