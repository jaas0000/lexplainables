...
Poort van `wetsanalyse-ai/tools/graph-qa/agent/graph/results.py`, 1:1 (werkwijze-story 047).
"""
...
def parse_select(tsv: str) -> list[dict[str, str]]:
    ...
    # Pel die buitenste laag er eerst af; kreeg je al kale TSV (met echte tabs), sla dit dan over.
    if raw.startswith('"'):
        try:
    ...
    for line in lines[1:]:
        ...
        rows.append(
            {
                header[i]: (_decode_term(cells[i]) if i < len(cells) else "")
