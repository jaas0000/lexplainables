...
Eén bron voor de annotatie-corpus: `queries.get_artikel` op GraphDB, geparseerd via
`parse_select`. Een latere API-laag-story kan dezelfde functie hergebruiken voor de
documentpaneel-weergave (`GET /v1/artikel`, nog niet gebouwd) — zie
`docs/project/stories/047-graph-qa-annotatie-enkele-ronde.md` §Afwijkingen punt 5.

Poort van `wetsanalyse-ai/tools/graph-qa/agent/artikel.py`, 1:1 minus `haal_artikel_sync`
(werkwijze-story 047).
"""
...
def _controleer_vindplaats(bwb_id: str, artikel: str, lid: str | None) -> None:
    """Kan dit überhaupt een bepaling aanduiden? Zo nee: een tikfout, en dat is iets anders dan
    niets gevonden. De query-bouwers valideren streng; hier vragen we ze dat alvast, vóór er een
    SPARQL de deur uit gaat."""
    try:
    ...
    for bouwer in (queries._art, queries._nummer_vrij):
        ...
        raise OngeldigeVindplaats(
            f"Ongeldige aanduiding: {artikel!r} (verwacht bv. '9', '22a' of '9.1')."
        )
...
def _leden_en_corpus(
    ...
    try:
        ...
        rows = []  # bv. artikel "9.1" geweigerd door get_artikel → straks de bepaling-fallback
