...
Kern van de juridische betrouwbaarheid: bronnen komen uit wat de graaf daadwerkelijk terugstuurde,
NIET uit de prozatekst van het model. Zo passeert een gehallucineerde citatie niet als "bron", en
gaat een echte vindplaats niet verloren als het model de IRI parafraseert zonder hem uit te typen.

De citatie-herkenning (`iter_refs`) wordt gedeeld met `agent/grounding.py`, dat dezelfde patronen
op de antwoordtekst toepast om niet-onderbouwde verwijzingen te markeren. We herkennen:
...
De vocabulaire-namespace (`ONTOLOGIE`) valt er bewust buiten: dat zijn predicaten, geen
vindplaatsen.

Poort van `wetsanalyse-ai/tools/graph-qa/agent/provenance.py`, 1:1 (werkwijze-story 044).
"""
...
def iter_refs(text: str) -> Iterator[tuple[str, str | None, str | None]]:
    ...
    Een kale BWB-id levert alleen een verwijzing op als hij niet al binnen een gevonden IRI/jci
    valt.
    """
    seen: set[str] = set()
