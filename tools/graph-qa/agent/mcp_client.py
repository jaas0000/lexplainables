"""MCP HTTP client voor de GraphDB MCP-server (werkwijze-story 040).

Implementeert het MCP Streamable HTTP transport protocol:
  - POST /mcp voor initialize / tools/list / tools/call
  - Authorization: Bearer *** — de server stuurt initialize als plain JSON, overige calls als SSE
    (text/event-stream) die direct sluit na het eerste event.
Gebruikt `httpx.post()` (geen streaming) — de server sluit de verbinding zelf.

Eén persistente `httpx.Client` wordt hergebruikt over alle calls (connection pooling; scheelt een
TCP+TLS-handshake per tool-aanroep). `close()` sluit die client af.

Veiligheidsnet: `call_tool` weigert SPARQL-argumenten die eruitzien als een update
(INSERT/DELETE/LOAD/CLEAR/DROP/CREATE). De echte read-only-garantie hoort aan de serverkant; dit
is defense-in-depth zolang het model nog rauwe SPARQL kan sturen.

Poort van `wetsanalyse-ai/tools/graph-qa/agent/mcp_client.py`, 1:1.
"""
...
# herkend, dus `PREFIX x: <http://a/> LOAD <http://evil/x.ttl>` liep er zo doorheen. Nu is de
# vraag omgedraaid — wat overblijft na het strippen van commentaar, PREFIX en BASE moet met een
# leesvorm beginnen. Alles wat daar niet aan voldoet wordt geweigerd, ook wat we nog niet kennen.
...
# bevatten ("nr. #3") en vrijwel elke prefix-IRI eindigt erop (<urn:bwb-ns:>). Beide vormen
# matchen daarom éérst, zodat alleen een echt losstaand '#' als commentaar wordt gestript.
...
_DECLARATIE_RE = re.compile(r"^\s*(?:base\s*<[^>]*>|prefix\s+[^\s:]*:\s*<[^>]*>)\s*", re.IGNORECASE)


def _query_kern(query: str) -> str:
    """De query zonder commentaar en zonder PREFIX-/BASE-declaraties — wat er echt wordt
    uitgevoerd."""
    zonder_commentaar = _STRING_OF_HASH.sub(
        lambda m: "" if m.group(0).startswith("#") else m.group(0), query or ""
    )
...
def _looks_like_update(query: str) -> bool:
    ...
    Bewust streng: het GraphDB-service-account achter de auth-proxy mág schrijven op de
    repository, dus dit vangnet is in de praktijk wat een schrijf-SPARQL vanuit de agent
    tegenhoudt. Bij twijfel weigeren kost hooguit een tool-foutmelding die het model kan
    herstellen; doorlaten kan de graaf kosten.
    """
...
class MCPClient:
    ...
    def _rpc(self, method: str, params: dict[str, Any] | None = None) -> Any:
        # Lazy handshake: deze GraphDB-versie (11.4.0, live geverifieerd) weigert `tools/call`
        # zonder eerst een sessie te openen via `initialize` (400 `McpError` zonder sessie-header).
        # De referentie-app riep `initialize()` nergens expliciet aan vóór een tool-call — kennelijk
        # was dat bij hun MCP-server-versie niet nodig. Één keer per client-instantie, hier i.p.v.
        # bij elke aanroeper, zodat `sparql()`/`semantic_search()` zelf niets van sessies hoeven te
        # weten.
        if method != "initialize" and self._session_id is None:
            self.initialize()
        ...
        if "text/event-stream" in ct:
            ...
            # Non-2xx zonder JSON-RPC-result → expliciet falen i.p.v. stil een leeg resultaat
            # teruggeven.
    ...
    def sparql(self, query: str) -> str:
        ...
        De read-only guard hoort hier — op de SPARQL-string — en niet in het generieke
        `call_tool`: anders zou hij ook de natuurlijke-taal-query van `similarity_search` scannen
        en die onterecht weigeren zodra ze met een verb begint of iets als 'delete where' bevat.
        """
    ...
    def close(self) -> None:
        # Idempotent en best-effort: bij een SSE-client-disconnect kan `close()` samenvallen met
        # een nog lopende call in een executor-thread; laat het sluiten daar niet op stuklopen.
