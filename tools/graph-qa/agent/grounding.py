...
2. **Citaten** — tekst die het antwoord tússen aanhalingstekens zet, moet letterlijk in de
   opgehaalde tekst staan. De agent belooft letterlijk te citeren, en een citaat met één woord
   verschil is precies waar een jurist op afgaat.
...
class GroundingReport:
    ...
    # Hoeveel passages er als citaat zijn nagelopen — geslaagd én mislukt. Zonder dit getal is
    # niet te melden wát er is gecontroleerd: een antwoord dat artikelen in gewone taal noemt
    # ("artikel 2 lid 1 onderdeel m") heeft nul vindplaatsen, en dan las de tijdlijn "0
    # verwijzingen onderbouwd" — terwijl er twee citaten wél waren getoetst en klopten.
    citaten: int = 0
...
def check_grounding(answer_text: str, source_trace: list[tuple[str, str]]) -> GroundingReport:
    ...
    niet_letterlijk = [
        c for c in citaten if "\\" not in c and not komt_letterlijk_voor(trace_text, c)
    ]
...
def curate_sources(sources: list[Source], answer_text: str) -> list[Source]:
    ...
    Coarse op BWB-id zodat alle relevante artikel-/lid-IRI's van een besproken regeling behouden
    blijven, terwijl opgehaalde-maar-onbesproken regelingen wegvallen. Valt terug op de volledige
    lijst als het antwoord geen enkel BWB-id noemt (dan niets weggooien).
    """
