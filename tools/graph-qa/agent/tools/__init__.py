"""Getypeerde domein-toollaag over de kennisgraaf (werkwijze-story 041).

Het model kiest voortaan uit deze bewerkingen i.p.v. vrije SPARQL te schrijven: onze code bouwt
de query deterministisch (`agent/graph/queries.py`) en voert die uit via de `GraphPort`. Elke
tool draagt in zijn beschrijving het "hoe"; de correctheid zit in geteste code, niet in
prompt-proza. `raw_sparql` blijft als gated escape.
...
Poort van `wetsanalyse-ai/tools/graph-qa/agent/tools/__init__.py`, 1:1.
"""
...
TOOLS: list[dict[str, Any]] = [
        ...
        "input_schema": _obj(
            {
                ...
                "limit": {
                    "type": "integer",
                    "description": "Max. aantal treffers (1-50, default 10).",
                },
            },
            ["query"],
        ),
        ...
        "input_schema": _obj(
            {
                "query": {
                    "type": "string",
                    "description": "Natuurlijke omschrijving van wat je zoekt.",
                },
        ...
        "input_schema": _obj(
            {
                "bwb_id": _BWB,
                "artikel": _ART,
                "lid": {"type": "string", "description": "Lidnummer, bijv. '1'."},
            },
            ["bwb_id", "artikel", "lid"],
        ),
        "handler": _h_get_lid,
    },
    {
        ...
        "description": (
            ...
            "'25', '22a') én voor beleidsregels/circulaires met decimale nummers zoals '9.1' (bv. "
            "de Leidraad Invordering 2008), waar get_artikel/get_lid niet passen."
        ),
        "input_schema": _obj(
            {
                "bwb_id": _BWB,
                "nummer": {
                    "type": "string",
                    "description": "Bepaling-nummer, bijv. '9.1' of '22a'.",
                },
            },
            ["bwb_id", "nummer"],
        ),
    ...
    },
    {
        ...
        "input_schema": _obj(
            {"term": {"type": "string", "description": "Begrip of deel ervan."}}, ["term"]
        ),
    ...
    },
    {
        ...
        "input_schema": _obj(
            {"query": {"type": "string", "description": "SPARQL SELECT/CONSTRUCT/DESCRIBE."}},
            ["query"],
        ),
...
def dispatch(name: str, graph: GraphPort, args: dict[str, Any] | None, settings: Any = None) -> str:
    ...
    try:
        ...
        # Verwachte fouten (ongeldig argument, MCP-fout, ontbrekende sleutel): als tekst
        # teruggeven.
        return f"Fout bij tool '{name}': {exc}"
    except httpx.HTTPError as exc:
        # Transport-/statusfout naar de graaf (timeout, connection-reset): NIET de hele beurt
        # breken — geef 'm als tool-resultaat terug zodat de agent kan herstellen/rapporteren.
        logger.warning("tool '%s' netwerkfout naar de graaf", name, exc_info=True)
        return (
            f"Fout bij tool '{name}': de kennisgraaf was tijdelijk onbereikbaar "
            f"({type(exc).__name__})."
        )
