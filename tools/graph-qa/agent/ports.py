...
De loop praat uitsluitend via deze interfaces, zodat tests een fake kunnen injecteren zonder
netwerk, en zodat een tweede LLM-/graaf-provider later een kwestie van een nieuwe adapter is.
"""
...
class GraphPort(Protocol):
    ...
    De domeintools bouwen SPARQL en voeren die uit via `sparql()`; de loop hoeft het rauwe
    MCP-tooloppervlak niet te kennen.
    """
...
class LLMPort(Protocol):
    ...
    Retourneert het provider-native response-object (blokken met .type/.text/.id/.name/.input en
    een .stop_reason); de loop leest daar direct uit, en een fake kan hetzelfde vormpje
    teruggeven.
    """
...
class LLMStream(Protocol):
    ...
    `text_deltas` levert de tekst-brokjes zodra ze binnenkomen; `final_message()` geeft na afloop
    het volledige response-object (blokken + stop_reason).
    """
