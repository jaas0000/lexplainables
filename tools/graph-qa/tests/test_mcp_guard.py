...
De guard werkte in een eerdere versie met een blocklist van update-sleutelwoorden, en die was te
omzeilen: LOAD, CLEAR en DROP werden alleen aan het begin van een REGEL herkend, dus achter een
PREFIX op dezelfde regel liepen ze er doorheen. De `BYPASS`-gevallen hieronder zijn precies die
vormen; ze horen bij een allowlist vanzelf te sneuvelen.

Poort van `wetsanalyse-ai/tools/graph-qa/tests/test_mcp_guard.py`, 1:1 (werkwijze-story 040).
"""
...
# Vormen die een regelbegin-blocklist doorliet omdat het verb niet aan het regelbegin stond.
BYPASS = [
...
def test_updates_herkend(q: str) -> None:
    assert _looks_like_update(q) is True


@pytest.mark.parametrize("q", BYPASS)
def test_update_achter_een_prefix_wordt_ook_geweigerd(q: str) -> None:
    """De regressie: een regelbegin-blocklist keek naar het begin van de regel, dus dit glipte
    erdoor."""
    assert _looks_like_update(q) is True


@pytest.mark.parametrize("q", BENIGN)
def test_benigne_queries_niet_herkend(q: str) -> None:
    assert _looks_like_update(q) is False


def test_reject_updates_gooit_mcperror() -> None:
    with pytest.raises(MCPError):
        MCPClient._reject_updates({"query": "INSERT DATA { <a> <b> <c> }"})


def test_reject_updates_laat_select_door() -> None:
...
def test_semantic_search_verb_query_niet_geweigerd(monkeypatch) -> None:
    # De guard hoort op de SPARQL-string, niet op elk tool-argument. Een natuurlijke-taal-query
    # voor similarity_search die met een verb begint of 'insert data' bevat, mag niet stuklopen.
    c = _client(monkeypatch)
    for q in (
        "insert data over de BRP",
        "Add-on regelingen bij de IW",
        "clear inzicht in verjaring",
    ):
        assert c.semantic_search(q) == ""  # geen MCPError


def test_raw_sparql_update_nog_steeds_geweigerd(monkeypatch) -> None:
