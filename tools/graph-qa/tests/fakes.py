...
def make_settings(**kw: Any) -> Settings:
    """Settings voor tests, met sensibele overrides waar tests anders steeds hetzelfde zouden
    moeten meegeven."""
    return Settings(**kw)
...
class FakeLLM:
    ...
    def _leg_vast(self, kwargs: dict[str, Any]) -> None:
        ...
        De aanroeper mag het systeemblok gesplitst aanleveren (`[stabiel, variabel]`) zodat de
        echte adapter er een prompt-cache-punt tussen kan zetten; wat het model uiteindelijk leest
        is de aaneenschakeling. Tests vragen naar dát — de splitsing zelf staat in `system_delen`.
        """
        systeem = kwargs.get("system")
        if isinstance(systeem, list):
            kwargs = {
                **kwargs,
                "system_delen": systeem,
