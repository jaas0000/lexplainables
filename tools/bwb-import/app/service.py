...
- `POST /import`  -> importeer één regeling (`bwb_id`) of een batch (`bwb_ids`); per wet
  idempotent, fouten per wet gerapporteerd.

De service publiceert bewust geen hostpoort in `deploy/graphdb/`: importeren is een schrijfactie
op de graaf en wordt van binnen het `graphdb_default`-netwerk aangeroepen. Optioneel beveiligd met
een `X-API-Key`-header (`BWB_SERVICE_API_KEY_FILE`, werkwijze-ADR-0006).
"""
...
class ImportRequest(BaseModel):
    """Aanvraag voor een import. `bwb_ids` (batch) heeft voorrang boven `bwb_id` (enkel)."""
    ...
    def _valideer(self) -> ImportRequest:
        ...
        if self.bwb_ids is None and not self.bwb_id:
            raise ValueError("geef bwb_id of bwb_ids op")
        return self
...
@app.post("/import")
async def importeer(body: ImportRequest, x_api_key: str | None = Header(default=None)) -> dict:
    settings: Settings = app.state.settings
    _check_api_key(settings, x_api_key)

    if body.bwb_ids is not None:
    ...
    bwb_id = body.bwb_id
    assert bwb_id is not None  # afgedwongen door ImportRequest._valideer
