...
function log(niveau, bericht, velden = {}) {
    process.stderr.write(JSON.stringify({ ts: new Date().toISOString(), niveau, bericht, ...velden }) + "\n");
}
// ── API-client ──────────────────────────────────────────────────────────────
export async function apiFetch(method, path, body) {
    ...
    try {
        ...
        throw new Error(`API niet bereikbaar (${method} ${path}): ${e.message}`);
    }
...
export const TOOLS = [
    ...
    },
    {
        ...
        run: ({ titel, inhoud, type, versie }) => apiFetch("POST", "/v1/admin/berichten", {
            titel,
            inhoud,
            type,
            versie: versie ?? null,
    ...
    },
    {
        ...
        run: ({ id, gepubliceerd }) => apiFetch("PATCH", `/v1/admin/berichten/${id}/publicatie`, {
            gepubliceerd,
        }),
    },
];
...
async function main() {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (req) => {
        ...
        try {
            ...
            return {
                content: [{ type: "text", text: `Fout: ${e.message}` }],
                isError: true,
            };
        }
    });
